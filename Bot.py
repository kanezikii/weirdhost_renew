...
def add_server_time(server_url="https://hub.weirdhost.xyz/server/1cbf7c50"):
    ...
    with sync_playwright() as p:
        ...
        try:
            ...
            # 2. 读取当前续期冷却状态 (XX시간 후에 연장할수있어요 / 지금 연장이 가능해요)
            ...
            if status_locator.count() > 0:
                ...
                if "지금" in status_text or "가능해요" in status_text:
                    ...
                    time.sleep(10)
                    browser.close()
                    return True # 成功点击，返回 True
                else:
                    print("⏳ 判断结果：【冷却中】。无需点击。")
                    print("🎉 任务正常结束（由于未到时间，不进行点击，这不是报错）。")
                    browser.close()
                    return True # 未到时间，不应视为脚本失败，返回 True 保持 Actions 绿灯
            else:
